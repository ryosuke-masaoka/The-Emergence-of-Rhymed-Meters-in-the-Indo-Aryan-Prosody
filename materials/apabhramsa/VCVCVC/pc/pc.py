...
for line in file:
	...
	line = re.sub('-', '', line)
	line = line.rstrip()
	m_iter = re.finditer('[aeiouāãĕīïĩŏũüūōēẽ][^aeiouāãĕīïĩŏũüūōēẽ\s]*[aeiouāãĕīïĩŏũüūōēẽ][^aeiouāãĕīïĩŏũüūōēẽ\s]*[aeiouāãĕīïĩŏũüūōēẽ][^aeiouāãĕīïĩŏũüūōēẽ$]*\s' , line)
	for m in m_iter:
		mrhyme = m.group()
		mrhyme = mrhyme.rstrip()
		rhymepattern.append(mrhyme)
	o_iter = re.finditer('[aeiouāãĕīïĩŏũüūōēẽ][^aeiouāãĕīïĩŏũüūōēẽ\s]*[aeiouāãĕīïĩŏũüūōēẽ][^aeiouāãĕīïĩŏũüūōēẽ\s]*[aeiouāãĕīïĩŏũüūōēẽ][^aeiouāãĕīïĩŏũüūōēẽ\s]*\$', line)
	for o in o_iter:
		orhyme = o.group()
		orhyme = re.sub('\$', '', orhyme)
		endrhyme.append(orhyme)
	n = re.search('[aeiouāãĕīïĩŏũüūōēẽ][^aeiouāãĕīïĩŏũüūōēẽ\s]*[aeiouāãĕīïĩŏũüūōēẽ][^aeiouāãĕīïĩŏũüūōēẽ\s]*[aeiouāãĕīïĩŏũüūōēẽ][^aeiouāãĕīïĩŏũüūōēẽ\s]*$' , line)
	if n:
